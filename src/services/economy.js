...
const rl = (s, key) => s?.research?.nodes?.[key] || 0; //research level helper
...
export const effectivePerTap = (s) => ( (s?.perTap || 0) + tapFlat(s) ) * globalMultiplier(s) * tapBoost(s);

export const effectiveCps = (s) => ( (s?.cps || 0) + cpsFlat(s) ) * globalMultiplier(s) * cpsMult(s) * cpsBoost(s);

//compute tap amount with combo & crit; combo step is boosted by research
export const computeTapOutcome = (s, now = Date.now()) => {
  ...
  //Crit chance includes research; clamp to 30%
