...
const art = (s) => summarizeArtifacts(s?.artifacts?.equipped || []);
...
export const tokensForRun = (runTotalFed) => Math.floor(Math.pow(Math.max(0, runTotalFed) / 10000, 0.6));
...
//Event multiplier (Frenzy) + artifact bonus
const eventMult = (s) => {
  const base = (s?.events?.frenzyUntil || 0) > Date.now() ? 1.5 : 1;
  return base * (1 + art(s).frenzyMult);
};

//Research + artifacts
const tapFlat = (s) => rl(s, 'tapPower') + art(s).flatTap;
const cpsFlat = (s) => rl(s, 'cpsPower');
const cpsMult = (s) => (1 + 0.10 * rl(s, 'cpsMulti')) * (1 + art(s).cpsMult);
const comboStepBonus = (s) => 0.01 * rl(s, 'comboStep') + art(s).comboStep;
const critBonus = (s) => Math.min(0.2, 0.2 * rl(s, 'critChance') + art(s).critChance);

export const effectivePerTap = (s) => ((s?.perTap || 0) + tapFlat(s)) * globalMultiplier(s) * eventMult(s) * tapBoost(s);

export const effectiveCps = (s) => ((s?.cps || 0)) * globalMultiplier(s) * eventMult(s) * cpsMult(s) * cpsBoost(s);

//compute tap amount with combo & crit 
export const computeTapOutcome = (s, now = Date.now()) => {
  const step = (s?.combo?.step ?? 0.05) + comboStepBonus(s);
